 /* ═══════════════════════════════════════════
     히어로 섹션 초록 그라데이션
     ═══════════════════════════════════════════ */
// Raw WebGL2 / WebGPU mode — no 2D context claim (would lock the canvas).
    var canvas = document.getElementById('canvas');
    var __ctArea = canvas.parentElement;
    var __maxDim = (window.innerWidth <= 768) ? 960 : 4096;

    // Inject preserveDrawingBuffer + alpha so export (toDataURL) captures the GL frame.
    ...
    var controls = (function() {
      var _v = {"sourceImage":"images/hero/1.jpg","color1":"#125622","color2":"#87BD4F","color3":"#C0F083","color4":"#FFFFFF","playing":true,"speed":1.1,"grainSpeed":0,"sharpness":0,"flow":1,"noise":0.01,"opacity":0.80,"backdropBlur":30,"eraserOn":true,"eraseRadius":50,"eraserSoftness":1,"eraserFade":3};
    ...
    {
      // @clay-no-bg-control
// Fluid Grain Overlay — WebGL2. The background image is rendered INSIDE
// the shader so it can be blurred under the fluid (CSS backdrop-filter
// cannot blur an element's own background). Erased areas reveal the sharp
// image; covered areas show it blurred by the Backdrop Blur amount.
...
function ctrl(key, fallback) {
  try { const v = controls.get(key); return (v === undefined || v === null) ? fallback : v; }
  catch (e) { return fallback; }
}

// --------------------------------------------------------------- MASK FBO
let maskW = 0, maskH = 0;
let maskFbo = [null, null];
let maskTex = [null, null];
let maskRead = 0;
...
void main(){
  ...
  vec2 a=uPoint;     a.x*=uAspect;
  vec2 b=uPrevPoint; b.x*=uAspect;
  float d=segDist(p,a,b);
  float innerFrac=mix(0.95,0.02,clamp(uSoftness,0.0,1.0));
  float add=1.0-smoothstep(uRadius*innerFrac,uRadius,d);
  float acc=max(prev.r,add);
...
uniform float uBlur;      // blur radius in uv units
in vec2 vUv;
out vec4 fragColor;
...
// cover-fit the bg image into the canvas uv space
vec2 coverUv(vec2 uv){
  vec2 c=uv-0.5;
  float r=uCanvasAspect/uBgAspect;
  if(r>1.0) c.x*=r; else c.y/=r;
  return c+0.5;
}
// gaussian-ish blur of the background
vec3 blurBg(vec2 uv,float rad){
  if(rad<=0.0) return texture(uBg,coverUv(uv)).rgb;
  vec3 sum=vec3(0.0); float wsum=0.0;
  for(int i=-3;i<=3;i++){
    for(int j=-3;j<=3;j++){
      vec2 o=vec2(float(i),float(j))/3.0*rad;
      float w=exp(-(float(i*i+j*j))*0.35);
      sum+=texture(uBg,coverUv(uv+o)).rgb*w;
      wsum+=w;
    }
  }
  return sum/wsum;
...
void main(){
  ...
  float n1=snoise(vec3(p*uFlow,t));
  float n2=snoise(vec3(p*uFlow+vec2(5.2,1.3),t+10.0));
  float n3=snoise(vec3(p*uFlow+vec2(-3.1,4.2),t+20.0));
  float n4=snoise(vec3(p*uFlow+vec2(2.5,-2.1),t+30.0));

  float grainTime=uTime*uGrainSpeed;
  float grain=hash(uv*uResolution+grainTime*10.0)*2.0-1.0;

  float edge0=mix(-0.2,0.45,uSharpness);
  float edge1=mix(0.8,0.55,uSharpness);
  n1=smoothstep(edge0,edge1,n1+grain*uNoise);
  n2=smoothstep(edge0,edge1,n2+grain*uNoise);
  n3=smoothstep(edge0,edge1,n3+grain*uNoise);
  n4=smoothstep(edge0,edge1,n4+grain*uNoise);

  // PREMULTIPLIED accumulation: rgb is always color*coverage, so it can
  // NEVER drift toward black at thin edges -> no gray edge.
  vec3 pc=vec3(0.0);   // premultiplied color
  float a=0.0;         // coverage / alpha
  pc+=uColor1*n1*(1.0-a); a+=n1*(1.0-a);
  pc+=uColor2*n2*(1.0-a); a+=n2*(1.0-a);
  pc+=uColor3*n3*(1.0-a); a+=n3*(1.0-a);
  pc+=uColor4*n4*(1.0-a); a+=n4*(1.0-a);

  // Vignette + opacity scale coverage uniformly (rgb & a stay in lock-step).
  float vignette=length(uv-0.5)*2.0;
  float mfac=1.0-vignette*0.3;
  mfac*=uOpacity;
  // Erase mask reveals background where erased.
  float mask=texture(uMask,uv).r;
  mfac*=(1.0-mask);
  mfac=clamp(mfac,0.0,1.0);

  pc*=mfac;
  float fa=clamp(a*mfac,0.0,1.0);

  if(uHasBg>0.5){
    // Background blurred where fluid covers, sharp where erased.
    vec3 sharpBg=texture(uBg,coverUv(uv)).rgb;
    vec3 softBg=blurBg(uv,uBlur*fa);
    vec3 bg=mix(sharpBg,softBg,fa);
    // Fluid (premultiplied) over background -> opaque output.
    vec3 outRgb=pc+bg*(1.0-fa);
    fragColor=vec4(outRgb,1.0);
  } else {
    // No bg loaded yet -> transparent premultiplied fluid.
...
const U = {
  time: gl.getUniformLocation(fluidProg, 'uTime'),
  res: gl.getUniformLocation(fluidProg, 'uResolution'),
  c1: gl.getUniformLocation(fluidProg, 'uColor1'),
  c2: gl.getUniformLocation(fluidProg, 'uColor2'),
  c3: gl.getUniformLocation(fluidProg, 'uColor3'),
  c4: gl.getUniformLocation(fluidProg, 'uColor4'),
  speed: gl.getUniformLocation(fluidProg, 'uSpeed'),
  grainSpeed: gl.getUniformLocation(fluidProg, 'uGrainSpeed'),
  flow: gl.getUniformLocation(fluidProg, 'uFlow'),
  noise: gl.getUniformLocation(fluidProg, 'uNoise'),
  sharp: gl.getUniformLocation(fluidProg, 'uSharpness'),
  opacity: gl.getUniformLocation(fluidProg, 'uOpacity'),
  mask: gl.getUniformLocation(fluidProg, 'uMask'),
  bg: gl.getUniformLocation(fluidProg, 'uBg'),
  hasBg: gl.getUniformLocation(fluidProg, 'uHasBg'),
  bgAspect: gl.getUniformLocation(fluidProg, 'uBgAspect'),
  canvasAspect: gl.getUniformLocation(fluidProg, 'uCanvasAspect'),
  blur: gl.getUniformLocation(fluidProg, 'uBlur'),
};
const B = {
  prev: gl.getUniformLocation(brushProg, 'uPrev'),
  point: gl.getUniformLocation(brushProg, 'uPoint'),
  prevPoint: gl.getUniformLocation(brushProg, 'uPrevPoint'),
  radius: gl.getUniformLocation(brushProg, 'uRadius'),
  aspect: gl.getUniformLocation(brushProg, 'uAspect'),
  softness: gl.getUniformLocation(brushProg, 'uSoftness'),
};
const DEC = {
  prev: gl.getUniformLocation(decayProg, 'uPrev'),
  decay: gl.getUniformLocation(decayProg, 'uDecay'),
};
...
function loadBgImage(src) {
  ...
  currentBgSrc = src;
  const img = new Image();
  // data: URLs (user uploads) don't need/accept crossOrigin; remote URLs do.
  ...
  img.onerror = () => { console.error('Source Image failed to load:', src); };
  img.src = src;
}

// Initial load from the Source Image control.
loadBgImage(ctrl('sourceImage', ''));
// Reload whenever the user picks a new image.
try { controls.onChange('sourceImage', (v) => loadBgImage(v)); } catch (e) {}
...
function stamp(uvNow) {
  ...
  gl.bindFramebuffer(gl.FRAMEBUFFER, maskFbo[write]);
  gl.viewport(0, 0, maskW, maskH);
  gl.disable(gl.BLEND);
  gl.useProgram(brushProg);
  bindQuad(brushProg);
  gl.activeTexture(gl.TEXTURE0);
  gl.bindTexture(gl.TEXTURE_2D, maskTex[maskRead]);
  gl.uniform1i(B.prev, 0);
  gl.uniform2f(B.point, uvNow.x, uvNow.y);
  const prevP = lastUv || uvNow;
  gl.uniform2f(B.prevPoint, prevP.x, prevP.y);
  gl.uniform1f(B.radius, ctrl('eraseRadius', 40) / canvas.width);
  gl.uniform1f(B.aspect, canvas.width / canvas.height);
  gl.uniform1f(B.softness, ctrl('eraserSoftness', 0.6));
...
canvas.addEventListener('pointerdown', (e) => {
  if (!ctrl('eraserOn', false)) return;
  pointerDown = true; lastUv = null;
  stamp(eventUv(e));
});
canvas.addEventListener('pointermove', (e) => {
  if (!pointerDown || !ctrl('eraserOn', false)) return;
  stamp(eventUv(e));
});
window.addEventListener('pointerup', () => { pointerDown = false; lastUv = null; });

try { controls.onAction('clearEraser', clearMask); } catch (e) {}
...
function render(now) {
  ...
  let playing = true;
  try { playing = controls.get('playing'); } catch (e) {}
  if (playing) time += dt * 0.001;

  // Mask decay -> heal erased areas over Eraser Fade Time (seconds to ~0).
  const fadeT = ctrl('eraserFade', 3);
  if (fadeT > 0) {
    const dtSec = Math.min(dt * 0.001, 0.05);
    const perSecond = Math.pow(0.01, 1.0 / fadeT);
    const decay = Math.pow(perSecond, dtSec);
    const wDec = 1 - maskRead;
    gl.bindFramebuffer(gl.FRAMEBUFFER, maskFbo[wDec]);
    gl.viewport(0, 0, maskW, maskH);
    gl.disable(gl.BLEND);
  ...
  gl.uniform3fv(U.c1, hexToRgb(ctrl('color1', '#ff007f')));
  gl.uniform3fv(U.c2, hexToRgb(ctrl('color2', '#FFF300')));
  gl.uniform3fv(U.c3, hexToRgb(ctrl('color3', '#B800FF')));
  gl.uniform3fv(U.c4, hexToRgb(ctrl('color4', '#ff99ff')));
  gl.uniform1f(U.speed, ctrl('speed', 1));
  gl.uniform1f(U.grainSpeed, ctrl('grainSpeed', 0));
  gl.uniform1f(U.flow, ctrl('flow', 0.5));
  gl.uniform1f(U.noise, ctrl('noise', 0.14));
  gl.uniform1f(U.sharp, ctrl('sharpness', 0));
  gl.uniform1f(U.opacity, ctrl('opacity', 1));

  // Background: blur amount (px) -> uv-space radius.
  const blurPx = ctrl('backdropBlur', 8);
  gl.uniform1f(U.hasBg, bgLoaded ? 1.0 : 0.0);
  gl.uniform1f(U.bgAspect, bgAspect);
  gl.uniform1f(U.canvasAspect, canvas.width / canvas.height);
  gl.uniform1f(U.blur, blurPx / Math.max(canvas.width, canvas.height) * 4.0);
...
window.addEventListener('resize', resizeCanvas);
window.addEventListener('load', resizeCanvas); // 이미지 로드 완료 후 크기 재계산
if (typeof ResizeObserver !== 'undefined') {
  new ResizeObserver(resizeCanvas).observe(__ctArea); // 실시간 크기 변화 감지
}
resizeCanvas();

var controls = (function() {
  // 💡 투명도 조절: 아래 코드에서 "opacity":0.95 부분의 숫자를 조절하세요! (1.0 = 진함, 0.5 = 반투명)
  var _v = {"sourceImage":"images/hero/1.jpg","color1":"#125622","color2":"#87BD4F","color3":"#C0F083","color4":"#FFFFFF","playing":true,"speed":1.1,"grainSpeed":0,"sharpness":0,"flow":1,"noise":0.05,"opacity":0.95,"backdropBlur":40,"eraserOn":true,"eraseRadius":78,"eraserSoftness":1,"eraserFade":3};
  var _defaults = JSON.parse(JSON.stringify(_v));
  var _listeners = {};
  var _anyListeners = [];
  var _actionListeners = {};
  ...
  // Custom getMousePos to match the new structure
...
uniform vec2 uGyro; // 💡 자이로스코프 좌표 수신용
in vec2 vUv; out vec4 fragColor;
...
void main(){
  ...
  // 💡 플루이드 노이즈 좌표에만 자이로스코프 간섭 적용 (배경 이미지는 고정)
...
const U = {
  ...
  gyro: gl.getUniformLocation(fluidProg, 'uGyro') // 💡 셰이더로 보낼 자이로 변수 추가
};
const B = {
